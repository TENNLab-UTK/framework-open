# This creates an subtraction network in tmp_subtraction.txt.  It takes two values
...
if [ $# -ne 4 ]; then
  echo 'usage: sh scripts/subtraction_run.sh v0 v1 w os_framework' >&2
  exit 1
fi
...
# Make the subtraction network.  Info is going to be in tmp_info.txt.

sh $fro/scripts/subtraction_network.sh $w $fro > tmp_subtraction.txt
...
if [ $cs = $diff ]; then correct=1; else correct=0; fi
...
echo "Difference:" $cs
echo "Computed-Difference: $diff"
echo "Correct: $correct"
echo "Overflow: $overflow"
echo "Underflow: $underflow"

echo ""
echo "The network is in tmp_subtraction.txt"
