...
for i in $t ; do
  ...
  if [ ! -d testing/vrisp/$i ]; then
    echo "Error -- no directory testing/vrisp/$i" >&2
    exit 1
  fi

  for f in label.txt network_tool.txt processor.sh processor_tool.txt correct_output.txt ; do
    if [ ! -f testing/vrisp/$i/$f ]; then
      echo "Error -- no file testing/vrisp/$i/$f" >&2
      exit 1
    fi
  done
  ...
  l=`cat testing/vrisp/$i/label.txt`
  sh testing/vrisp/$i/processor.sh > tmp_proc_params.txt
  ...
  bin/network_tool < testing/vrisp/$i/network_tool.txt > tmp_nt_output.txt 2>&1
  if [ `wc tmp_nt_output.txt | awk '{ print $1 }'` != 0 ]; then
    echo "Test $i - $l" >&2
    echo "There was an error in the network_tool command when I ran:" >&2
    echo "" >&2
    echo "bin/network_tool < testing/vrisp/$i/network_tool.txt > tmp_nt_output.txt" >&2
    echo "" >&2
  ...
  cp testing/vrisp/$i/processor_tool.txt tmp_pt_input.txt

  bin/processor_tool_vrisp < testing/vrisp/$i/processor_tool.txt > tmp_pt_output.txt 2> tmp_pt_error.txt

  if [ `wc tmp_pt_error.txt | awk '{ print $1 }'` != 0 ]; then
    echo "Test $i - $l" >&2
    echo "There was an error in the processor_tool_risp command when I ran:" >&2
    echo "" >&2
    echo "bin/processor_tool_vrisp < testing/vrisp/$i/processor_tool.txt" >&2
    echo "" >&2
    cat tmp_pt_error.txt >&2
    exit 1
  fi

  d=`diff tmp_pt_output.txt testing/vrisp/$i/correct_output.txt | wc | awk '{ print $1 }'`
  if [ $d != 0 ]; then
    echo "Test $i - $l" >&2
    echo "Error: Output does not match the correct output." >&2
    echo "       Output file is tmp_pt_output.txt" >&2
    echo "       Correct output file is testing/vrisp/$i/correct_output.txt" >&2
    exit 1
  fi
