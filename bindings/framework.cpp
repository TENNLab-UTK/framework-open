...
PYBIND11_MODULE(neuro, m) {
	...
	py::class_<neuro::int_hash>(m, "int_hash")
		.def(py::init<>())
		.def("__call__", &int_hash::operator(), "Hashes an integer value");

	py::class_<neuro::coord_hash>(m, "coord_hash")
		.def(py::init<>())
		.def("__call__", &coord_hash::operator(), "Hashes a coordinate value");
