...
void print_commands(FILE* f) {
    ...
    fprintf(f, "This is a processor tool program. The commands listed below "
               "are case-insensitive,\n");
    fprintf(f, "For commands that take a json either put a filename on the "
               "same line,\n");
    fprintf(
        f, "or the json can be multiple lines, starting on the next line.\n\n");

    fprintf(f, "Action commands --\n");

    fprintf(f, "MAKE/M proc_name proc_param_json    - Make a new processor "
               "with no network\n");
    fprintf(f, "LOAD/L network_json                 - Load a network on the "
               "processor\n");
    fprintf(f, "ML network_json                     - Make a new processor "
               "from the network & load the network.\n");

    fprintf(f, "AS node_id spike_time spike_val ... - Apply normalized spikes "
               "to the network (note: node_id, not input_id)\n");
    fprintf(f, "ASV node_id spike_time spike_val .. - Apply unnormalized "
               "spikes to the network (note: node_id, not input_id)\n");
    fprintf(f, "ASR node_id spike_raster_string     - Apply spike raster to "
               "the network (note: node_id, not input_id)\n");
    fprintf(f, "RUN simulation_time                 - Run the network for "
               "\"simulation_time\" cycles\n");
    fprintf(f, "RSC/RUN_SR_CH sim_time [node] [...] - Run, and then print "
               "spike raster and charge information in columns\n");
    fprintf(f, "CLEAR-A/CA                          - Clear the network's "
               "internal state \n");
    fprintf(f, "CLEAR/C                             - Remove the network from "
               "processor\n");

    fprintf(f, "\nOutput tracking info commands --\n");
    fprintf(f, "OLF [node_id] [...]                 - Output the last fire "
               "time for the given output or all outputs\n");
    fprintf(f, "OC [node_id] [...]                  - Output the spike count "
               "for the given output or all outputs\n");
    fprintf(f, "OT/OV [node_id] [...]               - Output the spike times "
               "for the given output or all outputs\n");
    fprintf(f, "TRACK_O [node_id] [...]             - Track output events for "
               "given outputs (empty=all)\n");
    fprintf(f, "UNTRACK_O [node_id] [...]           - Untrack output events "
               "for given output (empty=all)\n");
    ...
    fprintf(f, "TNC                                 - Total fire count for all "
               "neurons.\n");
    fprintf(f, "TNA                                 - Total accumulates for "
               "all neurons.\n");
    fprintf(f, "NT/NV show_nonfiring(T/F)           - All firing times for "
               "tracked neurons.\n");
    fprintf(f, "GSR [T/F]] [node] [...]             - Print spike raster info "
               "for tracked neurons.\n");
    fprintf(f, "NCH [node_id] [...]                 - Print the charges "
               "(action potentials) of the neurons (empty=all).\n");
    fprintf(f, "NLFJ                                - Print the neuron last "
               "fire json\n");
    fprintf(
        f,
    ...
    fprintf(f, "TRACK_N [node_id] [...]             - Track neuron events for "
               "specified neurons (empty=all)\n");
    fprintf(f, "UNTRACK_N [node_id] [...]           - Untrack neuron events "
               "for specified neurons (empty=all)\n");
    fprintf(f, "SW [from to]                        - Show synapse weights (or "
               "just one synapse).\n");
    fprintf(f, "PULL_NETWORK file                   - Pull the network off the "
               "processor and store in  file.\n");
    fprintf(f, "\n");

    fprintf(f, "Other info commands --\n");
    fprintf(f, "PARAMS [file]                       - Print the JSON that can "
               "recreate the processor\n");
    fprintf(f, "NP/PPACK                            - Print the PropertyPack "
               "that networks use with the processor.\n");
    fprintf(
        f,
        "NAME                                - Print the processor's name\n");
    fprintf(f, "EMPTYNET [file]                     - Create an empty network "
               "for this processor\n");
    fprintf(f, "PP                                  - Print processor's "
               "properties - (not a universal feature).\n");
    fprintf(f, "INFO                                - Print the network's node "
               "ids and output tracking info\n");
    fprintf(f, "PS                                  - Print the spikes we have "
               "applied\n");
...
int node_validation(const Network* n, const string& node_id) {
    ...
    if (!n->is_node(nid))
        throw SRE(node_id + " is not a node in the network");
    return nid;
}
    ...
    try {
        if (normalize) {
            if (s.value < -1 || s.value > 1)
                throw "spike val must be >= -1 and <= 1";
        }
        if (s.time < 0)
            throw "spike time must be > 0";
...
Network* load_network(Processor** pp, const json& network_json) {
    ...
    //     cerr << "neuro::Network::track_all_neuron_events not supported on "
    //             "platform "
    //          << p->get_name() << endl;
    // throw SRE("track_all_neuron_events() failed."); // TODO JDM This is
    // stated as not needed in the markdown, but throws an error if not
    // implemented?
...
int main(int argc, char** argv) {
    ...
    vector<vector<double>>
        all_output_times; // hold return value of output_vectors()
    vector<vector<double>>
        neuron_times; // hold the return value of neuron_times();
    vector<string>
        spike_strings; // hold spike strings from neuron_vectors_to_json()
    vector<int> v;
    ...
    while (1) {
        try {
            ...
            if (sv.size() == 0 || sv[0][0] == '#') {
                ...
                if (network_processor_validation(net, p)) {
                    if (sv.size() < 2 || (sv.size() - 1) % 3 != 0) {
                        printf("usage: %s node_id spike_time spike_value "
                               "node_id1 spike_time1 spike_value1 ...\n",
                               sv[0].c_str());
                    } else {
                        ...
                        for (i = 0; i < (sv.size() - 1) / 3; i++) {
                            try {
                                ...
                                if (sscanf(sv[i * 3 + 1].c_str(), "%d",
                                    ...
                                    throw SRE((string) "Invalid spike [ " +
                                              sv[i * 3 + 1] + "," +
                                              sv[i * 3 + 2] + "," +
                                              sv[i * 3 + 3] + "]\n");
                                }
                                spike_validation(
                                    Spike(spike_id, spike_time, spike_val), net,
                                    normalize);

                                p->apply_spike(
                                    Spike(net->get_node(spike_id)->input_id,
                                          spike_time, spike_val),
                                    normalize);
                                spikes_array.push_back(
                                    Spike(spike_id, spike_time, spike_val));
                        ...
                        try {
                            ...
                            for (i = 0; i < sv[2].size(); i++) {
                                if (sv[2][i] != '0' && sv[2][i] != '1') {
                                    throw SRE("ASR -- Spike raster string must "
                                              "be only 0's and 1's.");
                                }
                            ...
                            apply_spike_raster(
                                p, net->get_node(spike_id)->input_id, sr);
                ...
                for (i = 0; i < spikes_array.size(); i++) {
                    printf("Spike: [%d,%lg,%lg]\n", spikes_array[i].id,
                           spikes_array[i].time, spikes_array[i].value);
                }
                ...
                if (network_processor_validation(net, p)) {
                    if (sv.size() != 2 ||
                        sscanf(sv[1].c_str(), "%lf", &sim_time) != 1 ||
                        sim_time < 0) {
                ...
                if (network_processor_validation(net, p)) {
                    if (sv.size() == 1 ||
                        ...
                        if (gsr_nodes.empty()) {
                            for (i = 0; i < net->sorted_node_vector.size();
                                 i++) {
                                gsr_nodes.insert(
                                    net->sorted_node_vector[i]->id);
                            }
                        }
                        ...
                        // Print spike raster and charge information for each
                        // timestep
                ...
                /* the behavior of Caspian and GNP is different.
                   Caspian will get the sum to running time. GNP will get the
                   simulation time
                */
                if (network_processor_validation(net, p))
                    printf("time: %.1lf\n", p->get_time());
                ...
                if (sv.size() == 1 || (sv[1] != "T" && sv[1] != "F")) {
                    printf("usage: NLF show_nonfiring - Last fire times for "
                           "all neurons. show_nonfiring=T/F\n");
                } else if (network_processor_validation(net, p)) {
                    net->make_sorted_node_vector();
                    output_times = p->neuron_last_fires();
                    if (output_times.size() == 0) {
                        printf("Recording last fire times for neurons is not "
                               "implemented by %s.\n",
                               p->get_name().c_str());
                    }
                ...
                */
                ...
                if (sv.size() == 1 || (sv[1] != "T" && sv[1] != "F")) {
                    printf("usage: NC show_nonfiring - Fire counts for all "
                           "neurons. show_nonfiring=T/F\n");
                } else if (network_processor_validation(net, p)) {
                    net->make_sorted_node_vector();
                    event_counts = p->neuron_counts();
                    if (event_counts.size() == 0) {
                        printf("Recording event counts for neurons is not "
                               "implemented by %s.\n",
                               p->get_name().c_str());
                    }
                ...
                if (network_processor_validation(net, p)) {
                    ...
                    if (charges.size() == 0) {
                        printf("Recording charges for neurons is not "
                               "implemented by %s.\n",
                               p->get_name().c_str());
                    }
                    ...
                    for (i = 0; i < charges.size(); i++) {
                        ...
                        if (v.size() == 0 ||
                            (j < v.size() && v[j] == node_id)) {
                            printf("Node %*s charge: %lg\n", max_name_len,
                                   node_name(node).c_str(), charges[i]);
                            j++;
                        }
                    }
                }
                ...
                if (sv.size() != 1 && sv.size() != 3) {
                    printf("usage: SW [from to] - show weights for all "
                           "synapses, or just one synapse.\n");
                } else if (network_processor_validation(net, p)) {
                    if (sv.size() == 3) {
                    ...
                    /* Since the previous call is O(synapses anyway), I'm not
                       embarrased of this loop when from/to are specified. */

                    for (i = 0; i < pres.size(); i++) {
                        if (from == -1 ||
                            (from == (int)pres[i] && to == (int)posts[i])) {
                            printf("  %4u -> %4u : %7.4lf\n", pres[i], posts[i],
                                   weights[i]);
                        }
                    }
                ...
                if (sv.size() != 2) {
                    printf("usage: PULL_NETWORK file - pull network from the "
                           "processor to the file.\n");
                } else if (network_processor_validation(net, p)) {
                    fout.clear();
                    fout.open(sv[1]);
                    if (fout.fail()) {
                ...
                if (sv.size() == 1 || (sv[1] != "T" && sv[1] != "F")) {
                    printf("usage: NV show_nonfiring - Fire counts for all "
                           "neurons. show_nonfiring=T/F\n");
                } else if (network_processor_validation(net, p)) {
                    net->make_sorted_node_vector();
                    try {
                        ...
                        if (neuron_times.size() == 0) {
                            printf("Recording events for neurons is not "
                                   "implemented by %s.\n",
                                   p->get_name().c_str());
                        } else {
                            for (i = 0; i < net->sorted_node_vector.size();
                                 i++) {
                                node_id = net->sorted_node_vector[i]->id;
                                if (neuron_times[i].size() > 0 ||
                                    sv[1][0] == 'T') {
                                    printf("Node %2u fire times:", node_id);

                                    for (j = 0; j < neuron_times[i].size();
                                         j++) {
                       ...
                       "OLF") { // test output_last_fire and output_last_fires.
                ...
                if (network_processor_validation(net, p)) {
                    ...
                    if (sv.size() == 1) {
                        ...
                        for (i = 1; i < sv.size(); i++) {
                            try {
                                if (sscanf(sv[i].c_str(), "%d", &node_id) !=
                                    1) {
                                    throw SRE(sv[i] +
            ...
            } else if (sv[0] == "TRACK_O" ||
                       sv[0] == "UNTRACK_O") { // track_output_events() and
                                               // track_all_output_events()
                if (network_processor_validation(net, p)) {
                    if (sv.size() == 1) {
                        if (sv[0][0] == 'T') {
                        ...
                        for (i = 1; i < sv.size(); i++) {
                            try {
                                ...
                                output_id = net->get_node(node_id)->output_id;
                                if (!p->track_output_events(
                                        output_id, (sv[0][0] == 'T'))) {
                                    snprintf(buf, 50, "%d, %d) failed.",
                                             output_id, (sv[0][0] == 'T'));
                                    throw SRE((string) "track_output_events(" +
                                              buf);
                                }
                            } catch (const SRE& e) {
                                printf("%s\n", e.what());
            ...
            } else if (sv[0] == "TRACK_N" ||
                       sv[0] == "UNTRACK_N") { // track_neuron_events() and
                                               // track_all_neuron_events()
                if (network_processor_validation(net, p)) {
                    if (sv.size() == 1) {
                        if (sv[0][0] == 'T') {
                        ...
                        for (i = 1; i < sv.size(); i++) {
                            try {
                                ...
                                if (!p->track_neuron_events(
                                        node_id, (sv[0][0] == 'T'))) {
                                    snprintf(buf, 50, "%d, %d) failed.",
                                             node_id, (sv[0][0] == 'T'));
                                    throw SRE((string) "track_neuron_events(" +
                                              buf);
                                }
                            } catch (const SRE& e) {
                                printf("%s\n", e.what());
                       ...
                       sv[0] == "OV") { //  output_vector(int output_id, int
                                        //  network_id = 0)
                if (network_processor_validation(net, p)) {
                    ...
                    if (sv.size() == 1) {
                        try {
                            ...
                            if (all_output_times.size() == 0) {
                                throw SRE(
                                    "Processor error -- p->output_vectors "
                                    "returned a vector of size zero");
                            }
                            for (i = 0; i < (size_t)net->num_outputs(); i++) {
                                ...
                                printf("node %s spike times:",
                                       node_name(node).c_str());
                                for (j = 0; j < all_output_times[i].size();
                                     j++) {
                ...
                if (p == nullptr) {
                    ...
                    cout << p->get_processor_properties() << endl;
                       ...
                       sv[0] == "PPACK") { // get_network_properties
                if (p == nullptr) {
                    printf("Must make a processor first\n");
                } else {
                ...
                if (network_processor_validation(net, p)) {
                    ...
                    neuron_times = p->neuron_vectors();
                    spike_raster =
                        neuron_vectors_to_json(neuron_times, "S", net);
                    spike_strings =
                        spike_raster["Spikes"].get<vector<string>>();

                    for (i = 0; i < net->sorted_node_vector.size(); i++) {
                        node = net->sorted_node_vector[i];
                        if (gsr_nodes.size() == 0 ||
                            gsr_nodes.find(node->id) != gsr_nodes.end()) {
                            if (gsr_hidden_nodes || !node->is_hidden()) {
                                ...
                                printf("%-*s %s : %s\n", max_name_len,
                                       node_name(node).c_str(), id.c_str(),
                                       spike_strings[i].c_str());
                            }
                        }
                    }
                }
                       ...
                       "INFO") { // print input,hidden and output nodes id.
                ...
                if (network_processor_validation(net, p)) {
                    cout << neuron_counts_to_json(p->neuron_counts(), net)
                         << endl;
                }
                ...
                if (network_processor_validation(net, p)) {
                    cout << neuron_charges_to_json(p->neuron_charges(), net)
                         << endl;
                }
                ...
                if (sv.size() != 2 || (sv[1] != "V" && sv[1] != "S")) {
                    fprintf(stderr,
                            "NVJ type=V/S - Print the neuron vector json\n");
                    fprintf(stderr, "  V means \"Events\" key with vector of "
                                    "vector values.\n");
                    fprintf(stderr, "  S means \"Spikes\" key with vector of "
                                    "spike strings.\n");
                } else if (network_processor_validation(net, p)) {

                    try {
                        cout << neuron_vectors_to_json(p->neuron_vectors(),
                                                       sv[1], net)
                             << endl;
                    } catch (const SRE& e) {
                        printf("%s\n", e.what());
                ...
                if (network_processor_validation(net, p)) {
                    cout << neuron_last_fires_to_json(p->neuron_last_fires(),
                                                      net)
                         << endl;
                }

            } else {
                printf("Invalid command %s.  Use '?' to print a list of "
                       "commands.\n",
                       sv[0].c_str());
            }
        } catch (const SRE& e) {
