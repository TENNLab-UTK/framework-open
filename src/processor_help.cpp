...
json neuron_counts_to_json(const vector <int> &counts, Network *n)
{
  ...
  if (counts.size() != 0 && counts.size() != n->sorted_node_vector.size()) {
    throw SRE((string) "neuron_counts_to_json: counts vector size != # neurons in network.");
  }
...
json neuron_charges_to_json(const vector <double> &charges, Network *n)
{
  ...
  if (charges.size() != 0 && charges.size() != n->sorted_node_vector.size()) {
    throw SRE((string) "neuron_charges_to_json: charges vector size != # neurons in network.");
  }
  for (i = 0; i < n->sorted_node_vector.size(); i++) {
    neurons.push_back(n->sorted_node_vector[i]->id);
  }
...
/* If last_fires.size() == 0, then we assume that the processor does not implement
   neuron last fires.  Instead of flagging an error, we simply create the proper
   JSON where all of the last firing times are -1. */
...
json neuron_last_fires_to_json(const vector <double> &last_fires, Network *n)
{
  ...
  n->make_sorted_node_vector();
  if (last_fires.size() != 0 && last_fires.size() != n->sorted_node_vector.size()) {
    throw SRE((string) "neuron_last_fires_to_json: last_fires vector size != # neurons in network.");
  }
  for (i = 0; i < n->sorted_node_vector.size(); i++) {
    neurons.push_back(n->sorted_node_vector[i]->id);
  }
                            ...
                            Network *n)
{
  ...
  if (events.size() != 0 && events.size() != n->sorted_node_vector.size()) {
    throw SRE((string) "neuron_vectors_to_json: vectors size != # neurons in network.");
  }
  for (i = 0; i < n->sorted_node_vector.size(); i++) {
    neurons.push_back(n->sorted_node_vector[i]->id);
  }
...
Network *pull_network(Processor *p, Network *n, int network_id)
{
  ...
  for (i = 0; i < pres.size(); i++) {
    if (!n->is_node(pres[i])) {
      snprintf(buf, 100, "pull_network(): Synapse from %u - Node %u is not in the network.",
                   pres[i], pres[i]);
      throw SRE((string) buf);
    }
    if (!n->is_node(posts[i])) {
      snprintf(buf, 100, "pull_network(): Synapse to %u - Node %u is not in the network.",
                   posts[i], posts[i]);
      throw SRE((string) buf);
    }

    if (!n->is_edge(pres[i], posts[i])) {
      snprintf(buf, 100, "pull_network(): Synapse from %u to %u is not in the network.",
                   pres[i], posts[i]);
      throw SRE((string) buf);
    }
  }
